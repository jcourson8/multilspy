...
from .language_servers import LanguageServers
from .multilspy_config import MultilspyConfig, Language
from .multilspy_exceptions import MultilspyException, LSNotFoundException, LSNotSupportedException
...
class LanguageServer:
    ...
    @classmethod
    def create(cls, config: MultilspyConfig, logger: MultilspyLogger) -> "LanguageServer":
        """
        ...
        if config.code_language == Language.PYTHON:
            if config.language_server is LanguageServers.JEDI or config.language_server is None:
                from multilspy.language_servers.jedi_language_server.jedi_server import (
                    JediServer as LanguageServerImplementation
                )
            else: 
                logger.log(f"Language server {config.language_server} not supported", logging.ERROR)
                raise LSNotSupportedException(config.language_server)

        elif config.code_language == Language.JAVA:
            if config.language_server is LanguageServers.ECLIPSEJDTLS or config.language_server is None:
                from multilspy.language_servers.eclipse_jdtls.eclipse_jdtls import (
                    EclipseJDTLS as LanguageServerImplementation
                )
            else: 
                logger.log(f"Language server {config.language_server} not supported", logging.ERROR)
                raise LSNotSupportedException(config.language_server)
            ...
            if config.language_server is LanguageServers.RUSTANALYZER or config.language_server is None:
                from multilspy.language_servers.rust_analyzer.rust_analyzer import (
                    RustAnalyzer as LanguageServerImplementation
                )
            else: 
                logger.log(f"Language server {config.language_server} not supported", logging.ERROR)
                raise LSNotSupportedException(config.language_server)
            ...
            if config.language_server is LanguageServers.OMNISHARP or config.language_server is None:
                from multilspy.language_servers.omnisharp.omnisharp import (
                    OmniSharp as LanguageServerImplementation
                )
            else: 
                logger.log(f"Language server {config.language_server} not supported", logging.ERROR)
                raise LSNotSupportedException(config.language_server)
        ...
        if LanguageServerImplementation is not None:
            return LanguageServerImplementation(config, logger)
        else:
            raise MultilspyException("No language server implementation could be found for the given configuration")

    def __init__(
        self,
        config: MultilspyConfig,
        initialize_params: LSPTypes.InitializeParams,
        ...
        :param config: The Multilspy configuration.
        :param initialize_params: The initialization parameters for the language server.
        :param logger: The logger to use.
        :param repository_root_path: The root path of the repository.
        ...
        self.repository_root_path: str = initialize_params["rootPath"]
...
class SyncLanguageServer:
    ...
    @classmethod
    def create(
        cls, config: MultilspyConfig, logger: MultilspyLogger
    ) -> "SyncLanguageServer":
        """
        Creates a language specific LanguageServer instance based on the given configuration, and appropriate settings for the programming language.
        ...
        """
        return SyncLanguageServer(LanguageServer.create(config, logger))
